...
**THE MEDIA, LIVE-STREAM & FOLLOW-ME DOCTRINE (nine ratifications, Seth, 2026-07-17) — FOUNDATIONAL; EXTENDS the curated-portfolio + payments doctrine; FUTURE FLOORS on the EXISTING Artifact + exportChild + Event primitives, NO build now (slots named, not wired).**
**(1) HOSTED-OR-EMBEDDED CONTENT; leaders attach curriculum owned-or-not.** An Artifact's content is either **HOSTED** (our access-controlled storage) or **EMBEDDED/REFERENCED** (external, e.g. YouTube — the storage-ref IS the embed); provenance records **author/owner vs. attacher**. Group leaders are ENCOURAGED to create + attach curriculum/technique videos — their **own OR a found YouTube video** — to the group they lead; embeds fully supported for curriculum/public content.
**(2) STORE FOR AI ACCESS (the hosting split's rule).** The AI using content as source material (a worksheet from a tutor's video) needs the **RAW source** — available only when **HOSTED** (transcribe/frames/structure, P4), never via a YouTube embed (locked behind YouTube's API). So: content we OWN + want the AI to work with (a tutor's own curriculum video, a child's own footage) → **HOST**; a third-party found video → **EMBED** (reference only, limited AI access). A **minor's OWN private footage → our access-controlled hosting, NEVER an unlisted external link** (P7+P9; obscurity ≠ access control).
**(3) MEDIA STACK — Cloudflare Stream + R2 (cost/capability-checked).** Stream: LIVE (phone/camera RTMPS/SRT push; consented followers watch live; auto-records to a replay ~60s) + **free AI transcripts** (Workers AI) + downloadable raw MP4 (AI access) + signed-URL access control, co-located with R2 (pennies-cheap raw archival + max AI access) — ~$1–2/child/mo. Alternatives: Mux (best API + free transcripts, pricier, separate), Bunny Stream (cheapest, does live, verify maturity). Live-from-phone is a standard app build item. *(Vendor choice is a future gated floor.)*
**(4) MULTI-VIEWER LIVE IS NATIVE.** Many invited family members in different locations watch the **SAME live stream at once** — CDN one-to-many, no concurrent limit or per-viewer surcharge (pay total watch-time, pennies at family scale) — each via a signed/invited link (a **PRIVATE watch-party, never public**). Capture-once-fan-out at its richest: ONE live capture → family watch live anywhere + auto replay + Follow Me + coach queue + AI transcript.
**(5) SIGNAL-NOT-DURATION (the live/save separation).** LIVE viewing (generous, cheap, ephemeral) is **SEPARATE** from SAVED footage (permanent, storage cost, downstream); downstream value = **SIGNAL, not length**. **FREE AUTO-SAVE up to a THRESHOLD** (~20–30 min illustrative, tunable — a gymnastics meet / wrestling tournament): short sports free+effortless, all-day recording hits the ceiling (abuse cap), long sports capture signal not raw game. Past-threshold: default stop-auto-save + rely on clips; an **optional explicit paid "keep whole game"** (no surprise auto-charges); live session may auto-end as a light guard. Long-sport highlights: **TAP-TO-CLIP** during the stream (parent/coach/remote family taps "highlight" → ~30s saved) now; **auto-highlight montage** (cheer/motion/score detection) a future floor. Resolves "an upstream feature must never corrupt the downstream."
**(6) TWO-BUCKET SPORT-ATTRIBUTE CAPTURE.** Capture DERIVES from a few sport **ATTRIBUTES** set at group creation (team/individual, length, structure — **NOT per-sport code**; a new sport = its tags). **INDIVIDUAL (pure signal):** parent films/uploads/live-streams; short (gymnastics/wrestling) auto-save whole; long 1v1 (tennis) = pure signal, whole match to coach + Follow Me with auto-shutoff. **TEAM (signal = the whole):** ONE game filmed **under the COACH's permission** (a coach-cockpit feature, delegable per-game via a film-link; **if not via the coach, not done**) → serves as the live stream (all join; link shareable pre-event) + the coach-cockpit annotation source + **auto-publishes to EVERY athlete's Follow Me**; stored **ONCE** + one annotated version (dissolves the long-sport storage/noise problem). Team parents contribute **SHORT clips only** (5×1-min, not 1×5-min — signal) to their **OWN child's** Follow Me (game folder + photos), never to the coach. Highlight scrub-bar from tap-to-clip + rewatch ("most-replayed" self-building). Realizes groups-roles-cockpits + signup-assembles-cockpit (sport → permissions → cockpit).
**(7) FOLLOW ME = HONEST AUTO-GENERATED TRUTH (not parent-curated for the Academy).** The Academy **AUTO-GENERATES** a Follow Me page for every enrolled student-athlete (**enrollment IS consent**; no secondary consent); the parent does NOT curate what shows — it's the accurate, beautiful downstream picture of how the child is actually doing (pure **SIGNAL** for a prospective monthly giver; incentive-alignment toward formation — "a home gets tidied before guests"). **TRUTH-WITH-DIGNITY** (resolves P7 — honest and safe are one): (a) **AGGREGATE** scores/metrics, **NEVER an individual incident** (a single mistake never surfaced); (b) the parent controls **DISTRIBUTION, not content** (the family decides whether/to-whom to share — invite-only; the platform never shares for them; an unimpressive page simply isn't shared); the honest signal is NON-curated, and the ONLY parent-added content is a HARD-CAPPED media-allowance (~10 one-min videos + ~25 photos/child; FOL-10). The child's OWN view is growth-and-celebration, never a bare low score; NO cross-child ranking/leaderboard, ever. Granular incident detail stays in the trusted interior. **Accuracy is the whole value** (Michelin/Amazon; gaming destroys it). Formation lesson intended (Luke 16:10-12). A **standalone thin-upstream group does NOT auto-generate a page** (offered only when upstream exists).
**(8) THE WEEKLY-ESSENTIALS RATING.** Weekly, **each group leader rates every child** in their group on the CORE-6 essentials (Respect, Effort, Coachability, Perseverance, Kindness, Integrity — stored VERSIONED; part of staff's job), and **each PARENT is likewise required to rate their own child weekly**; seamless, ~25 sec, each 1–10 as **STARS**. The essentials score **SHOWN on Follow Me = the AVERAGE of the group-leaders' weekly ratings**; the parent's own rating is captured but EXCLUDED from the shown average (surfaced separately as a perception-gap insight); grades etc. = downstream curation. Fits stars-not-grades / weight-faithfulness / first-name-only / invite-only.
**(9) CONSENT = THE ONE PLATFORM SIGNUP CONSENT (not separate/per-sport).** Shared footage (team videos, individual matches with an opponent in frame) needs **NO separate consent** — competitions are **PUBLIC events** (public-space filming + posting needs no separate permission) — it is **one item DISCLOSED in the single signup consent package** (consent once to the whole package; never duplicate/track two consents). Communicate the whole package plainly at signup; once consented, the whole package is consented. **DISTINCTION (a definition, not a 2nd consent):** strong **cross-family isolation governs a child's PRIVATE record** (grades/individual work/honest record → own family + trusted adults); **SHARED PUBLIC-EVENT footage is a lighter thing** covered by the one consent (the "absolute line" was over-stated for shared public footage; it **STILL holds for private data**).
**Build implication:** NO build now — every capture/schema decision preserves the **raw hosted source** (for AI), the **signal-not-duration** separation, **per-child ownership + Follow-Me derivation**, and the **one-consent** model; the media/live/Follow-Me/ratings floors are woven in as we reach them. (Full text: `docs/SPEC.md` §1, `docs/ONE_PAGE.md`.)

**THE FOLLOW-ME CURATION & FOLLOWER-ANCHOR RATIFICATION (Seth, 2026-07-20) — FOUNDATIONAL; sharpens the 2026-07-17 media doctrine, corrects "parent-curated"/"$1-to-follow".** Follow Me = auto-generated NON-curated honest signal (faithfulness 10-star + essentials-stars + achievements + upstream footage) + a HARD-CAPPED parent media-allowance (~10 one-min videos + ~25 photos/child — the only parent-added content) + fully parent-controlled DISTRIBUTION (invite-only, default-private). The child's OWN view is growth-and-celebration, never a bare low score; NO cross-child ranking/leaderboard, ever. Essentials = the CORE 6 (Respect, Effort, Coachability, Perseverance, Kindness, Integrity), versioned; shown = leaders' weekly average, parent's own rating EXCLUDED (perception-gap insight). Follower = parent-vouch + FREE view via Google/Apple; $1 card-verify ONLY to become a monthly giver (identity-not-revenue, LEG-10). Governs FOL-1/2/4, adds FOL-9 (honest signal) + FOL-10 (capped allowance). NO build now beyond the Follow-Me S1 thin interior display. (Full text: `docs/SPEC.md` §1, `docs/ONE_PAGE.md`.)
