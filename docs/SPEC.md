...
- **LEG-04 — Data minimization *(corrected 2026-07-11 by the value-capture principle, §1)*.** Minimization means **not collecting data that serves no declared purpose** — NOT deleting data the consented educational purpose is served by keeping. Under capture-and-retain, a child's work (ink, images, audio) is **retained for the consented purpose** and protected in proportion to sensitivity: **raw ink is on by default** (family opt-out), and **audio is retainable for the consented purpose** (not auto-discarded after transcription). The single **verify-don't-store** exception is the identity-document image — verified, never stored (the identity-proof doctrine, LEG-04). Retention *and protection* windows SHALL be explicit per data class (LEG-05); deletion is departure/request/schedule-triggered (LEG-06/LEG-12), never a while-enrolled timer.
